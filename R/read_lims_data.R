...
#' @param sample_types NOT USED FOR THE MOMENT charactervector
#' met de types staal die je wil inlezen
#' c("Project", "DUP", "BLANK", "CONTROL", "PRBLANCO", "STANDARD")
#' Voor genetica heb je nog andere stalen als QC_METHOD, SUBSAMPLE, ...
#' @param include_non_reportable FALSE indien enkel rapporteerbare resultaten
#' gewenst zijn, TRUE indien je alle resultaten wenst
#' (opgelet dit kunnen er heel veel zijn), of een character vector
#' met talkens ANALYSENAAM__COMPONENTNAAM (dubbele underscore),
#' dus bv  c("ALK_OEP_W__Filtratie", "ALK_OEP_w__Opmerking")
...
read_lims_data <- function(connection,
  ...
  rep_crit_row <-
    which(template_information |> pull("Kolom") == "IsReportable=1")
  sql_add <- ""
  ...
  sql_query <- parse_sql_report_query(
