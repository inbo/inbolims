...
test_that("LIMS data retrieval and reporting workflow matches expected", {
  # 1. SKIP TEST IF NOT ON THE WORK NETWORK
  # This is the most critical part for your package checks.
  # `skip_if_not()` will halt execution of this test if the condition is FALSE.
  skip_if_not(is_on_inbo_domain(),
    message = paste0(
      "Skipping LIMS tests: ",
      "Not connected to the company Windows domain."
    )
  )
  ...
  expect_true(DBI::dbIsValid(connection),
    info = "The database connection should be valid."
  )

  # Test lims_sample_information() against the saved "golden" file
  staal_info <- lims_sample_information(connection,
    project = c("I-19W001-02")
  ) |>
    dplyr::filter(substring(LabSampleID, 1, 8) == "19-00010") |>
    dplyr::arrange(Project, LabSampleID)
  expected_staal_info <- readRDS("fixtures/expected_staal_info.rds")
  expect_equal(staal_info, expected_staal_info,
    info = "Sample information does not match the expected output."
  )
  ...
  expect_equal(rapport_data, expected_rapport_data,
    info = "Rapport data does not match the expected output."
  )
  print(getwd())
  ...
  expect_equal(staaloverzicht, expected_staaloverzicht,
    info = "Sample Overview does not match the expected output"
  )

  # Test lims_report_xtab() against the saved "golden" file
  kruistabel <- lims_report_xtab(rapport_data)
  expected_kruistabel <- readRDS("fixtures/expected_kruistabel.rds")
  expect_equal(kruistabel, expected_kruistabel,
    info = "Kruistabel does not match the expected output."
  )

  # Test lims_report_export() for both file creation and content
  lims_report_export(kruistabel, path = "test_xtab.csv")
  expect_true(file.exists("test_xtab.csv"),
    info = "Exported crosstab file should exist."
  )
  ...
  expect_equal(created_xtab_csv, expected_xtab_csv,
    info = "Exported xtab CSV content does not match expected."
  )

  # Test the second export for file existence
  lims_report_export(rapport_data, path = "test.csv")
  expect_true(file.exists("test.csv"),
    info = "Exported report data file should exist."
  )
})
