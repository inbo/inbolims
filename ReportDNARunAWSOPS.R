############################################################################
# SERVER SPECIFIC - AANPASSEN PER OMGEVING
############################################################################
...
setwd("L:\\LabWare-7\\Data\\Rscripts\\workdir")
basepath <- "L:\\LabWare-7\\Data\\SAMPLES_GENETICA\\REPORT"
logfile <- file.path(basepath, "ReportDNARun.log")
...
LIMSDESIGN <- data.frame(Replicate=LIMSDESIGN$ID, Specimen=LIMSDESIGN$Specimen,
                           Plate=factor(LIMSDESIGN$Plate, levels=unique(LIMSDESIGN$Plate)), Capilar=LETTERS[LIMSDESIGN$Capilar],
                           Lane=LIMSDESIGN$Lane, SampleType=LIMSDESIGN$SampleType, SampleNumber = LIMSDESIGN$SampleNumber, ParentSample=LIMSDESIGN$ParentSample, ParentAliquot=LIMSDESIGN$ParentAliquot, MilliQ = LIMSDESIGN$MilliQ, DNA = LIMSDESIGN$DNA)
...
Design <- LIMSDESIGN
Design$Plate <- factor(Design$Plate, levels = unique(Design$Plate)) #unique om volgorde te garanderen voor het geval er 10 of meer platen zouden zijn
...
Resultdata <- expand.grid(Plate = levels(Design$Plate), 
                          Capilar = LETTERS[1:8], Ixno = 1:8, 
                          Lane01='', Lane02='', Lane03='', Lane04='', Lane05='', Lane06='',
                          Lane07='', Lane08='', Lane09='', Lane10='', Lane11='', Lane12='', stringsAsFactors=F)
...
for(i in 1:nrow(Design)){
  ...
  if(!is.na(SampleType) & SampleType == "SUBSAMPLE") {
    ...
    for(k in 1:length(positions)){
      Resultdata[Resultdata$PlateSeq==Plate_Seq & Resultdata$Capilar==Capilar & Resultdata$Ixno == k+2, lane] <- positions[k]       
    } 
    
    
  } else  
    if (is.na(SampleType) | SampleType == "") {
...
#sqlSave(connect2, Resultdata,  "C_DNA_RUN_REPORT_RESULTS", append=T) #veroorzaakt crash
