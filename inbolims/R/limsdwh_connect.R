...
#' str(dfResults)
#' }
limsdwh_connect <- function(deployment = "prd"){
  if (deployment != "uat") {
    ...
    if(class(con) != "Microsoft SQL Server") print("Connectie niet gelukt. Ben je op het INBO netwerk of via VPN verbonden? Contacteer de database administrator")

  } else {
    #DBI lijkt niet graag te werken met referentiebeheer
    con <- RODBC::odbcDriverConnect(
      connection = "Driver={ODBC Driver 13 for SQL Server};Server=inbo-sql06-uat.inbouat.be,1435;Database=W0003_00_Lims;Trusted_Connection=yes;"
    )  
