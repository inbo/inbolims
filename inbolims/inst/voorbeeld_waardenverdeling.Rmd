...
Via de functie `lims_connect()`kan je een connectie tot stand brengen met het datawarehouse.
...
conn <- limsdwh_connect() #indien conn -1 bevat dan is de connectie niet geslaagd

# #ter illustratie (regel per regel uitvoeren, niet de 3 tegelijk, want interactief)
# k1 <- select_lims_object(conn, keuze = "Matrix")
# k2 <- select_lims_object(conn, keuze = "Analyse")
# k3 <- select_lims_object(conn, keuze = "Component", multiple = TRUE, analyse = k2)
# 
# 
# #Op basis van de selecties
# Data <- lims_get_results(conn, analysis = k2, matrix = k1, component = k3, 
...
De ingelezen gegevens bevinden zich nu in het R object *Data* (dit hebben we zo gedefinieerd in de bovenstaande code). Nu kunnen we met dit object bewerkingen uitvoeren zoals de data filteren, plotten, ...
...
#Filter de data zodat je enkel de component OXAL.Fe.DS overhoudt en steek die in DataW
DataW <- filter(Data, Component %in% c("OXAL.Fe.DS"))

ggplot(DataW, aes(x = AnalysisDate, y = ResultNumeric)) + geom_point()

ggplot(DataW, aes(x = seq_len(nrow(DataW)),y = ResultNumeric)) + geom_point() + geom_line() + 
...
ggplot(DataW, aes(x = ResultNumeric)) + geom_density() + 
  geom_vline(data = filter(DataKwantiel, kwantiel %in% c(0.025,0.25, 0.50 ,0.75, 0.975)), 
             aes(xintercept = waarde, color = factor(kwantiel))) + 
  scale_color_manual(name = "kwantiel",
                     values = c("0.025" = "red", "0.25" = "orange", "0.5" = "green4",
